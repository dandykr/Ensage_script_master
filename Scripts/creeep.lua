local sleepTick = 0
local target = nil
local scriptSetting = 1 -- if == 1 then stop, after aggro & unaggro, 
...
function MainBody( tick )
	...
	if IsKeyDown(32) then -- pressing "X" button to aggro
		sleepTick = tick + 100
	...
	if IsKeyDown(66) then -- pressing "C" button to un-aggro
		sleepTick = tick + 100
